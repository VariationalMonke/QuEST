...
{   
    ...
    size_t totalGPUMem;  // total shared memory
    size_t freeGPUMem;   // avaliable shared memory
    cudaMemGetInfo(&freeGPUMem, &totalGPUMem);
    freeGPUMem *= 0.8; // leave some room for other parameters

    printf("total GPU memory: %lld\n", totalGPUMem);
    printf("free GPU memory: %lld\n", freeGPUMem);

    //printf("checkpoint 1\n");

    long long int maxAmpPerRank = ceil(freeGPUMem/(2*sizeof(qreal))); // the max number of amps on gpu memory
    printf("max amplitude per rank:%lld\n", maxAmpPerRank);

    int roundsAlloc = (numAmpsPerRank +(maxAmpPerRank-1))/maxAmpPerRank; // the rounds of allocation needed

    long long int allocSize = numAmpsPerRank <= maxAmpPerRank ? numAmpsPerRank : maxAmpPerRank ;
    printf("allocSize is: %lld\n", allocSize);

    qureg->roundsAlloc = roundsAlloc;
    qureg->allocSize = allocSize;
    ...
    long long int hostAllocSize = roundsAlloc > 1 ? numAmpsPerRank - allocSize : 0;
    printf("Host allocSize is: %lld\n", hostAllocSize);
    validateMemoryAllocationSize(hostAllocSize, __func__);

    size_t arrSize = (size_t) (hostAllocSize * sizeof(qreal));
    qureg->stateVec.real =static_cast<qreal*> (malloc(arrSize));
    qureg->stateVec.imag =static_cast<qreal*> (malloc(arrSize));
    //printf("checkpoint 4\n");
    if (qureg->stateVec.real == NULL) {
        perror("Failed to allocate memory for qureg->stateVec.real");
    }
    printf("qureg->stateVec.real: %p\n", (void*)qureg->stateVec.real);
    printf("qureg->stateVec.imag: %p\n", (void*)qureg->stateVec.imag);

    cudaMalloc(&(qureg->deviceStateVec.real), qureg->allocSize*sizeof(qreal));
    cudaMalloc(&(qureg->deviceStateVec.imag), qureg->allocSize*sizeof(qreal));
...
void statevec_initZeroState(Qureg qureg)
{
    ...
    long long int ampsPerChunk = qureg.allocSize;

    qreal *tempHostReal = (qreal*) malloc(ampsPerChunk * sizeof(qreal));
    qreal *tempHostImag = (qreal*) malloc(ampsPerChunk * sizeof(qreal));    
    printf("checkpoint 1, amps total = %lld, amps per chunk = %lld\n", qureg.numAmpsTotal, ampsPerChunk);


    for(long long int i = 0; i < qureg.numAmpsTotal; i+=ampsPerChunk){
        long long int ampsInThisChunk = min(ampsPerChunk, qureg.numAmpsTotal - i);
        CUDABlocks = ceil((qreal)ampsInThisChunk/threadsPerCUDABlock);

        printf("i = %lld\n", i);

        size_t byteOffset = (i-ampsPerChunk);

        if(i >= ampsPerChunk) {
            cudaDeviceSynchronize();
            cudaMemcpy(qureg.deviceStateVec.real, qureg.stateVec.real + byteOffset, ampsInThisChunk * sizeof(qreal),cudaMemcpyHostToDevice);
            cudaMemcpy(qureg.deviceStateVec.imag, qureg.stateVec.imag + byteOffset, ampsInThisChunk * sizeof(qreal),cudaMemcpyHostToDevice);
            printf("current chunk loaded\n");
        }
        ...
        if(qureg.roundsAlloc > 1){
            cudaDeviceSynchronize();
            if(i == 0){
                cudaMemcpy(tempHostReal, qureg.deviceStateVec.real, ampsInThisChunk * sizeof(qreal), cudaMemcpyDeviceToHost);
                cudaMemcpy(tempHostImag, qureg.deviceStateVec.imag, ampsInThisChunk * sizeof(qreal), cudaMemcpyDeviceToHost);
                printf("GPU chunk moved to buffer\n");
            }else {
                cudaMemcpy(qureg.stateVec.real + byteOffset, qureg.deviceStateVec.real, ampsInThisChunk * sizeof(qreal),cudaMemcpyDeviceToHost);
                cudaMemcpy(qureg.stateVec.imag + byteOffset, qureg.deviceStateVec.imag, ampsInThisChunk * sizeof(qreal),cudaMemcpyDeviceToHost);
                printf("current chunked moved back\n");
            }

            if(i >= qureg.numAmpsTotal-ampsPerChunk){
                cudaMemcpy(qureg.deviceStateVec.real,tempHostReal, ampsInThisChunk * sizeof(qreal), cudaMemcpyHostToDevice);
                cudaMemcpy(qureg.deviceStateVec.imag,tempHostImag, ampsInThisChunk * sizeof(qreal), cudaMemcpyHostToDevice);
                printf("GPU chunk moved back\n");
            }
        }
    }
