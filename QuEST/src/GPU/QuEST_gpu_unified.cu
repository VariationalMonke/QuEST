...
void statevec_setAmps(Qureg qureg, long long int startInd, qreal* reals, qreal* imags, long long int numAmps) {
    ...
    memcpy(
        ...
        reals,
        numAmps * sizeof(*(qureg.stateVec.real))); 
        //cudaMemcpyHostToDevice);
    memcpy(
        qureg.stateVec.imag + startInd,
        imags,
        numAmps * sizeof(*(qureg.stateVec.imag)));
        //cudaMemcpyHostToDevice);
}
...
void densmatr_initClassicalState(Qureg qureg, long long int stateInd)
{
    ...
    densmatr_initClassicalStateKernel<<<CUDABlocks, threadsPerCUDABlock>>>(
        qureg.numAmpsPerChunk, 
        qureg.stateVec.real, 
        qureg.stateVec.imag, densityInd);
}
...
{   
    ...
    cudaStream_t *streams = new cudaStream_t[env.numRanks];
    for (int gpuID = 0; gpuID < env.numRanks; gpuID++) {
        cudaSetDevice(gpuID);
        cudaStreamCreate(&streams[gpuID]);
    }

    // allocate GPU memory
    cudaMallocManaged(&(qureg->stateVec.real), qureg->numAmpsTotal*sizeof(qreal));
    cudaMallocManaged(&(qureg->stateVec.imag), qureg->numAmpsTotal*sizeof(qreal));
    cudaMallocManaged(&(qureg->firstLevelReduction), ceil(qureg->numAmpsTotal/(qreal)REDUCE_SHARED_SIZE)*sizeof(qreal));
    cudaMallocManaged(&(qureg->secondLevelReduction), ceil(qureg->numAmpsTotal/(qreal)(REDUCE_SHARED_SIZE*REDUCE_SHARED_SIZE))*
            sizeof(qreal));

    // check gpu memory allocation was successful
    validateQuregGPUAllocation(qureg, env, __func__);

    // set prefered location to an even partition between each gpu
    for (int i = 0; i < numGPUs; i++) {
        cudaSetDevice(i);
        cudaMemAdvise(qureg->stateVec.real+(numAmpsPerRank * i *sizeof(qreal)), numAmpsPerRank*sizeof(qreal)), cudaMemAdviseSetPreferredLocation, i);
    }

    // prefetch memory
    for (int i = 0; i < numGPUs; i++) {
        cudaSetDevice(i);
        cudaMemPrefetchAsync(state_vector, size, i);
    }
...
void statevec_destroyQureg(Qureg qureg, QuESTEnv env)
{
    ...
    // Free GPU memory
    cudaFreeHost(qureg.stateVec.real);
    cudaFreeHost(qureg.stateVec.imag);
    cudaFreeHost(qureg.firstLevelReduction);
    cudaFreeHost(qureg.secondLevelReduction);
}
...
DiagonalOp agnostic_createDiagonalOp(int numQubits, QuESTEnv env) {
    ...
    cudaMallocManaged(&(op.real), arrSize);
    cudaMallocManaged(&(op.imag), arrSize);

    // check gpu memory allocation was successful
    validateDiagonalOpGPUAllocation(&op, env, __func__);

    // initialise GPU memory to zero
    cudaMemset(op.real, 0, arrSize);
    cudaMemset(op.imag, 0, arrSize);
...
void agnostic_destroyDiagonalOp(DiagonalOp op) {
    free(op.real);
    free(op.imag);
    cudaFreeHost(op.deviceOperator.real);
    cudaFreeHost(op.deviceOperator.imag);
}
...
void agnostic_initDiagonalOpFromPauliHamil(DiagonalOp op, PauliHamil hamil) {
    ...
    cudaMallocManaged(&d_pauliCodes, mem_pauliCodes);
    memcpy(d_pauliCodes, hamil.pauliCodes, mem_pauliCodes); //cudaMemcpyHostToDevice);
    
    qreal* d_termCoeffs;
    size_t mem_termCoeffs = hamil.numSumTerms * sizeof *d_termCoeffs;
    cudaMallocManaged(&d_termCoeffs, mem_termCoeffs);
    memcpy(d_termCoeffs, hamil.termCoeffs, mem_termCoeffs);//, cudaMemcpyHostToDevice);
    ...
    cudaFreeHost(d_pauliCodes);
    cudaFreeHost(d_termCoeffs);
}
...
QuESTEnv createQuESTEnv(void) {
    ...
    env.rank=0;
    //env.numRanks=1;
    cudaGetDeviceCount(&env.numRanks);
...
qreal statevec_getRealAmp(Qureg qureg, long long int index){
    qreal el=0;
    memcpy(&el, &(qureg.stateVec.real[index]), 
...
qreal statevec_getImagAmp(Qureg qureg, long long int index){
    qreal el=0;
    memcpy(&el, &(qureg.stateVec.imag[index]), 
...
void statevec_initBlankState(Qureg qureg)
{
    ...
    for (int gpuID = 0; gpuID < qureg.numChunks; gpuID++) {
...
void statevec_multiControlledMultiQubitUnitary(Qureg qureg, long long int ctrlMask, int* targs, int numTargs, ComplexMatrixN u)
{
    ...
    cudaMallocManaged(&d_targs, targMemSize);
    memcpy(d_targs, targs, targMemSize);// cudaMemcpyHostToDevice);
    
    // flatten out the u.real and u.imag lists
    int uNumRows = (1 << u.numQubits);
    qreal* uReFlat;  cudaMallocManaged(&uReFlat, uNumRows*uNumRows * sizeof *uReFlat);
    qreal* uImFlat;  cudaMallocManaged(&uImFlat, uNumRows*uNumRows * sizeof *uImFlat);
    long long int i = 0;
    ...
    cudaMallocManaged(&d_ampInds, numTargAmps*gridSize * sizeof *d_ampInds);
    cudaMallocManaged(&d_reAmps,  numTargAmps*gridSize * sizeof *d_reAmps);
    cudaMallocManaged(&d_imAmps,  numTargAmps*gridSize * sizeof *d_imAmps);
    
    // call kernel
    statevec_multiControlledMultiQubitUnitaryKernel<<<CUDABlocks,threadsPerCUDABlock>>>(
        qureg, ctrlMask, d_targs, numTargs, uReFlat, uImFlat, d_ampInds, d_reAmps, d_imAmps, numTargAmps);
        
    // free kernel memory
    cudaFreeHost(uReFlat);
    cudaFreeHost(uImFlat);
    cudaFreeHost(d_targs);
    //cudaFreeHost(d_uRe);
    //cudaFreeHost(d_uIm);
    cudaFreeHost(d_ampInds);
    cudaFreeHost(d_reAmps);
    cudaFreeHost(d_imAmps);
}
...
__global__ void densmatr_findProbabilityOfZeroKernel(
    ...
    qreal prob = qureg.stateVec.real[densityIndex];   // im[densityIndex] assumed ~ 0
...
qreal densmatr_findProbabilityOfZero(Qureg qureg, int measureQubit)
{
    ...
    qreal zeroProb;
    memcpy(&zeroProb, qureg.firstLevelReduction, sizeof(qreal));//, cudaMemcpyDeviceToHost);
    return zeroProb;
}
...
qreal statevec_findProbabilityOfZero(Qureg qureg, int measureQubit)
{
    ...
    if (qureg.numQubitsInStateVec == 1) {
        qreal amp;
        memcpy(&amp, qureg.stateVec.real, sizeof(qreal));//, cudaMemcpyDeviceToHost);
        stateProb += amp*amp;
        memcpy(&amp, qureg.stateVec.imag, sizeof(qreal));//, cudaMemcpyDeviceToHost);
        stateProb += amp*amp;
        return stateProb;
    }
    ...
    memcpy(&stateProb, qureg.firstLevelReduction, sizeof(qreal));//, cudaMemcpyDeviceToHost);
    return stateProb;
}
...
__global__ void statevec_calcProbOfAllOutcomesKernel(
    ...
    qreal prob = (
        qureg.stateVec.real[ampInd]*qureg.stateVec.real[ampInd] + 
        qureg.stateVec.imag[ampInd]*qureg.stateVec.imag[ampInd]);
...
void statevec_calcProbOfAllOutcomes(qreal* outcomeProbs, Qureg qureg, int* qubits, int numQubits) {
    ...
    cudaMallocManaged(&d_qubits, mem_qubits);
    memcpy(d_qubits, qubits, mem_qubits);//, cudaMemcpyHostToDevice);
    ...
    cudaMallocManaged(&d_outcomeProbs, mem_outcomeProbs);
    ...
    memcpy(outcomeProbs, d_outcomeProbs, mem_outcomeProbs);//, cudaMemcpyDeviceToHost);
    
    // free GPU memory
    cudaFreeHost(d_qubits);
    cudaFreeHost(d_outcomeProbs);
}
...
__global__ void densmatr_calcProbOfAllOutcomesKernel(
    ...
    qreal prob = qureg.stateVec.real[flatInd];   // im[flatInd] assumed ~ 0
...
__global__ void densmatr_calcInnerProductKernel(
    ...
    qreal prod = (
          a.stateVec.real[index]*b.stateVec.real[index] 
        + a.stateVec.imag[index]*b.stateVec.imag[index]);
...
qreal densmatr_calcInnerProduct(Qureg a, Qureg b) {
    ...
    qreal innerprod;
    memcpy(&innerprod, b.firstLevelReduction, sizeof(qreal));//, cudaMemcpyDeviceToHost);
    return innerprod;
}
...
Complex statevec_calcInnerProduct(Qureg bra, Qureg ket) {
    ...
    memcpy(&innerProdReal, bra.firstLevelReduction, sizeof(qreal));//, cudaMemcpyDeviceToHost);
    ...
    memcpy(&innerProdImag, bra.firstLevelReduction, sizeof(qreal));//, cudaMemcpyDeviceToHost);
...
__global__ void densmatr_calcFidelityKernel(Qureg dens, Qureg vec, long long int dim, qreal* reducedArray) {
    ...
    qreal* densReal = dens.stateVec.real;
    qreal* densImag = dens.stateVec.imag;
    qreal* vecReal  = vec.stateVec.real;
    qreal* vecImag  = vec.stateVec.imag;
...
qreal densmatr_calcFidelity(Qureg qureg, Qureg pureState) {
    ...
    qreal fidelity;
    memcpy(&fidelity, pureState.firstLevelReduction, sizeof(qreal));//, cudaMemcpyDeviceToHost);
    return fidelity;
}
...
qreal densmatr_calcHilbertSchmidtDistance(Qureg a, Qureg b) {
    ...
    while (numValuesToReduce > 1) {
        ...
        if (firstTime) {
             densmatr_calcHilbertSchmidtDistanceSquaredKernel<<<numCUDABlocks, valuesPerCUDABlock, sharedMemSize>>>(
                 a.stateVec.real, a.stateVec.imag, 
                 b.stateVec.real, b.stateVec.imag, 
    ...
    qreal trace;
    memcpy(&trace, a.firstLevelReduction, sizeof(qreal));//, cudaMemcpyDeviceToHost);
...
qreal densmatr_calcPurity(Qureg qureg) {
    ...
    while (numValuesToReduce > 1) {
        ...
        if (firstTime) {
             densmatr_calcPurityKernel<<<numCUDABlocks, valuesPerCUDABlock, sharedMemSize>>>(
                 qureg.stateVec.real, qureg.stateVec.imag, 
    ...
    memcpy(&traceDensSquared, qureg.firstLevelReduction, sizeof(qreal));//, cudaMemcpyDeviceToHost);
...
void densmatr_collapseToKnownProbOutcome(Qureg qureg, int measureQubit, int outcome, qreal outcomeProb) {
    ...
    densmatr_collapseToKnownProbOutcomeKernel<<<CUDABlocks, threadsPerCUDABlock>>>(
        outcomeProb, qureg.stateVec.real, qureg.stateVec.imag, numBasesToVisit,
...
__global__ void densmatr_mixDensityMatrixKernel(Qureg combineQureg, qreal otherProb, Qureg otherQureg, long long int numAmpsToVisit) {
    ...
    combineQureg.stateVec.real[ampInd] *= 1-otherProb;
    combineQureg.stateVec.imag[ampInd] *= 1-otherProb;
  
    combineQureg.stateVec.real[ampInd] += otherProb*otherQureg.stateVec.real[ampInd];
    combineQureg.stateVec.imag[ampInd] += otherProb*otherQureg.stateVec.imag[ampInd];
}
...
void densmatr_oneQubitDegradeOffDiagonal(Qureg qureg, int targetQubit, qreal dephFac) {
    ...
    densmatr_mixDephasingKernel<<<CUDABlocks, threadsPerCUDABlock>>>(
        dephFac, qureg.stateVec.real, qureg.stateVec.imag, numAmpsToVisit,
...
void densmatr_mixTwoQubitDephasing(Qureg qureg, int qubit1, int qubit2, qreal dephase) {
    ...
    densmatr_mixTwoQubitDephasingKernel<<<CUDABlocks, threadsPerCUDABlock>>>(
        dephFac, qureg.stateVec.real, qureg.stateVec.imag, numBackgroundStates, numAmpsToVisit,
...
void densmatr_mixDepolarising(Qureg qureg, int targetQubit, qreal depolLevel) {
    ...
    densmatr_mixDepolarisingKernel<<<CUDABlocks, threadsPerCUDABlock>>>(
        depolLevel, qureg.stateVec.real, qureg.stateVec.imag, numAmpsToVisit,
...
void densmatr_mixDamping(Qureg qureg, int targetQubit, qreal damping) {
    ...
    densmatr_mixDampingKernel<<<CUDABlocks, threadsPerCUDABlock>>>(
        damping, qureg.stateVec.real, qureg.stateVec.imag, numAmpsToVisit,
        part1, part2, part3, bothBits);
}
...
Complex statevec_calcExpecDiagonalOp(Qureg qureg, DiagonalOp op) {
    ...
    memcpy(&expecReal, qureg.firstLevelReduction, sizeof(qreal));//, cudaMemcpyDeviceToHost);
    ...
    memcpy(&expecImag, qureg.firstLevelReduction, sizeof(qreal));//, cudaMemcpyDeviceToHost);
...
void agnostic_setDiagonalOpElems(DiagonalOp op, long long int startInd, qreal* real, qreal* imag, long long int numElems) {
    ...
    memcpy(
        op.real + startInd, 
        real,
        numElems * sizeof(*(op.real)));
        //cudaMemcpyHostToDevice);
    memcpy(
        op.imag + startInd,
        imag,
        numElems * sizeof(*(op.imag)));
        //cudaMemcpyHostToDevice);
}
...
__global__ void statevec_applyPhaseFuncOverridesKernel(
    ...
    qreal c = cos(phase);
    qreal s = sin(phase);
    qreal re = qureg.stateVec.real[index];
    qreal im = qureg.stateVec.imag[index];

    // = {re[amp] cos(phase) - im[amp] sin(phase)} + i {re[amp] sin(phase) + im[amp] cos(phase)}
    qureg.stateVec.real[index] = re*c - im*s;
    qureg.stateVec.imag[index] = re*s + im*c;
}
 ...
 void statevec_applyPhaseFuncOverrides(
    ...
    cudaMallocManaged(&d_qubits, mem_qubits);      memcpy(d_qubits, qubits, mem_qubits);//, cudaMemcpyHostToDevice);
    cudaMallocManaged(&d_coeffs, mem_terms);       memcpy(d_coeffs, coeffs, mem_terms);//, cudaMemcpyHostToDevice);
    cudaMallocManaged(&d_exponents, mem_terms);    memcpy(d_exponents, exponents, mem_terms);//, cudaMemcpyHostToDevice);
    cudaMallocManaged(&d_overrideInds, mem_inds);  memcpy(d_overrideInds, overrideInds, mem_inds);//, cudaMemcpyHostToDevice);
    cudaMallocManaged(&d_overridePhases,mem_phas); memcpy(d_overridePhases, overridePhases, mem_phas);//, cudaMemcpyHostToDevice);
    ...
    cudaFreeHost(d_qubits);
    cudaFreeHost(d_coeffs);
    cudaFreeHost(d_exponents);
    cudaFreeHost(d_overrideInds);
    cudaFreeHost(d_overridePhases);
}
...
void statevec_applyMultiVarPhaseFuncOverrides(
    ...
    int* d_qubits;                  cudaMallocManaged(&d_qubits,           mem_qubits);
    qreal* d_coeffs;                cudaMallocManaged(&d_coeffs,           mem_coeffs);
    qreal* d_exponents;             cudaMallocManaged(&d_exponents,        mem_exponents);
    int* d_numQubitsPerReg;         cudaMallocManaged(&d_numQubitsPerReg,  mem_numQubitsPerReg);
    int* d_numTermsPerReg;          cudaMallocManaged(&d_numTermsPerReg,   mem_numTermsPerReg);
    long long int* d_overrideInds;  cudaMallocManaged(&d_overrideInds,     mem_overrideInds);
    qreal* d_overridePhases;        cudaMallocManaged(&d_overridePhases,   mem_overridePhases);

    // copy function args into GPU memory
    memcpy(d_qubits, qubits,                    mem_qubits);//,             cudaMemcpyHostToDevice);
    memcpy(d_coeffs, coeffs,                    mem_coeffs);//,             cudaMemcpyHostToDevice);
    memcpy(d_exponents, exponents,              mem_exponents);//,          cudaMemcpyHostToDevice);
    memcpy(d_numQubitsPerReg, numQubitsPerReg,  mem_numQubitsPerReg);//,    cudaMemcpyHostToDevice);
    memcpy(d_numTermsPerReg, numTermsPerReg,    mem_numTermsPerReg);//,     cudaMemcpyHostToDevice);
    memcpy(d_overrideInds, overrideInds,        mem_overrideInds);//,       cudaMemcpyHostToDevice);
    memcpy(d_overridePhases, overridePhases,    mem_overridePhases);//,     cudaMemcpyHostToDevice);
    ...
    size_t gridSize = (size_t) threadsPerCUDABlock * CUDABlocks;
    cudaMallocManaged(&d_phaseInds, numRegs*gridSize * sizeof *d_phaseInds);
    ...
    cudaFreeHost(d_coeffs);
    cudaFreeHost(d_exponents);
    cudaFreeHost(d_numQubitsPerReg);
    cudaFreeHost(d_numTermsPerReg);
    cudaFreeHost(d_overrideInds);
    cudaFreeHost(d_overridePhases);
    cudaFreeHost(d_phaseInds);
}
...
void statevec_applyParamNamedPhaseFuncOverrides(
    ...
    long long int* d_overrideInds;  cudaMallocManaged(&d_overrideInds,     mem_overrideInds);
    qreal* d_overridePhases;        cudaMallocManaged(&d_overridePhases,   mem_overridePhases);
    qreal* d_params = NULL;         if (numParams > 0) cudaMallocManaged(&d_params, mem_params);
    ...
    memcpy(d_overridePhases, overridePhases,    mem_overridePhases);//,     cudaMemcpyHostToDevice);
    if (numParams > 0)
        memcpy(d_params, params, mem_params);//, cudaMemcpyHostToDevice);
